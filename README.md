...
**Online tournaments** — single-elimination brackets for 4–16 human entrants (bots can't enter), started from the online lobby's **Create Tournament** row, which opens a bracket browser you can also reach fully by keyboard/gamepad; any tournament already open lists right there in the lobby too, alongside ordinary game rooms. Creating one first walks you through a ruleset screen — game mode, attack bubbles, chain reactions, aim guide, and colors — applied identically to every match in that bracket; join an open one while it's still in registration, and the organizer alone decides when to start (no unanimous ready-up required). Byes pad an odd-sized field up to the next bracket size (4, 8, or 16), every match is best of three, and the bracket screen shows each match's live state, countdown, and score. Leaving the tournament or disconnecting mid-match forfeits your current and any future match; withdrawing asks for confirmation first since it forfeits a match in progress. If both players in a match report different winners, that match goes **disputed** and stays that way until the server operator resolves it — see [SetupServer.md](SetupServer.md#online-tournaments) if you're running your own server.
