...
**Online tournaments** — single-elimination brackets for 4–16 human entrants (bots can't enter), started from the online lobby's **Tournaments** row, which opens a bracket browser you can also reach fully by keyboard/gamepad. Create one or join an open one while it's still in registration; once every entrant readies up the organizer starts it, and byes pad an odd-sized field up to the next bracket size (4, 8, or 16). Every match is best of three on a locked ruleset — Classic mode, two players, 8 colors, chain reactions off, aim guide off, mouse/touch aiming on — so no per-room setting can drift the bracket partway through, and the bracket screen shows each match's live state, countdown, and score. Leaving the tournament or disconnecting mid-match forfeits your current and any future match; withdrawing asks for confirmation first since it forfeits a match in progress. If both players in a match report different winners, that match goes **disputed** and stays that way until the server operator resolves it — see [SetupServer.md](SetupServer.md#online-tournaments) if you're running your own server.
