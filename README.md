...
**Follow a server:** press **F** on a server in either list (or tap the star at the left of its row) and it will notify your phone when a player joins, so a quiet server can tell you a game is starting rather than you checking it. The notification arrives with the app backgrounded or closed, and is suppressed while you're in the foreground. Needs the server operator to run the optional relay — see [SetupServer.md](SetupServer.md). Android delivery is live and verified end to end on real hardware; iOS is built the same way but needs a paid Apple Developer account to confirm — see [docs/PUSH_SETUP.md](docs/PUSH_SETUP.md).
