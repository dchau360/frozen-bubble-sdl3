...
([GPLv2 licensed](https://github.com/dchau360/frozen-bubble-sdl3/blob/main/LICENSE),
source at
...
writes IP address, nickname, and connect/disconnect times to a local server
log for operational purposes (abuse investigation, debugging). Per-nickname
match statistics (wins, bubbles fired/popped) are also persisted
server-side so they can be shown across sessions. **Anyone can run a
Frozen Bubble server** — the app ships with no server of its own and
fetches a community-maintained list of public servers on startup (see the
...
[docs/PUSH_SETUP.md](https://github.com/dchau360/frozen-bubble-sdl3/blob/main/docs/PUSH_SETUP.md)
