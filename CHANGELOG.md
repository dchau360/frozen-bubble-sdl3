...
- **Fixed the Discord round-result stats message misreporting touchscreen shots as mouse on WASM/itch.io.** WASM's in-game fire handling deliberately dispatches off a single synthetic mouse-click event to avoid double-firing on a real tap, which meant it always reported `fromTouch=false` regardless of whether the shot came from an actual mouse or a phone's touchscreen. A small bookkeeping flag (mirroring the existing menu-tap echo guard) now tracks the real preceding touch event so the input-device badge — in-game, in the lobby, and in Discord's round-result message — correctly shows touch for touch and mouse for mouse. No change to fire/aim/swipe handling itself.
...
- **Fixed two bugs found in live testing of tournaments above.** Winning the first round of a match used to disconnect the loser and force them to forfeit every remaining round of the bracket — a mismatch between the client (which expected up to five rounds per room) and the server (which tears a room down and opens a fresh one after every single round). And four of the five ruleset options an organizer could configure above — game mode, chain reaction, aim guide, and colors — were silently discarded in favor of fixed defaults (Classic, off, off, 8) at the start of every match regardless of what was actually picked; only attack-bubble mode ever took effect. Both are fixed: rooms and rounds now agree 1:1, and the organizer's chosen ruleset actually applies.
