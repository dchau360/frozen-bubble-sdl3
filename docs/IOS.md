...
- **Your own certificate**: sign the `.app` with a development or distribution
  identity and an embedded provisioning profile, then repackage. The app
  needs no custom entitlements of its own, but the installer still requires
  `application-identifier` and the team ID that Xcode would normally derive
  for you, so pull the profile's own entitlements out and sign with those
  (PlistBuddy needs a seekable file, not a pipe, hence the intermediate file):
  ```bash
