...
void circle_init(void)
{
    ...
    circle_steps = (int *)malloc(XRES * YRES * sizeof(int));
    if (!circle_steps)
        fb__out_of_memory();
...
void plasma_init(char *datapath)
{
    ...
    plasma2 = (unsigned char *)malloc(XRES * YRES);
    if (!plasma2)
        fb__out_of_memory();
    ...
    plasma3 = (unsigned char *)malloc(XRES * YRES);
    if (!plasma3)
        fb__out_of_memory();
}
            ...
            else
            {
                ...
                */
                ...
                r = CLAMP(a_recip * transform_cubic(dy,
                                                    ...
                                                    transform_cubic(dx, r_[12] * a_[12], r_[13] * a_[13], r_[14] * a_[14], r_[15] * a_[15])),
                          0, 255);
                g = CLAMP(a_recip * transform_cubic(dy,
                                                    ...
                                                    transform_cubic(dx, g_[12] * a_[12], g_[13] * a_[13], g_[14] * a_[14], g_[15] * a_[15])),
                          0, 255);
                b = CLAMP(a_recip * transform_cubic(dy,
                                                    ...
                                                    transform_cubic(dx, b_[12] * a_[12], b_[13] * a_[13], b_[14] * a_[14], b_[15] * a_[15])),
                          0, 255);
                set_pixel(dest, x, y, r, g, b, a);
            }
            x__ += cosval;
...
void snow_(SDL_Surface *dest, SDL_Surface *orig)
{
    ...
    if (flakes == NULL)
    {
        flakes = (flake *)malloc(sizeof(struct flake) * amount);
        if (!flakes)
            fb__out_of_memory();
        for (i = 0; i < amount; i++)
        {
...
void draw_line_(SDL_Surface *surface, int x1, int y1, int x2, int y2, SDL_Color *color)
{
    ...
    while (1)
    {
        x += xacc;
        y += yacc;
        if ((xacc == 1 && x > x2) || (xacc == -1 && x < x2) || (yacc == 1 && y > y2) || (yacc == -1 && y < y2))
        {
